...
function SocialLink({ href, label }: { href?: string; label: string }) {
  if (!href) return null;
  return (
    <a href={href} target="_blank" rel="noreferrer" className="text-sm text-slate-400 underline decoration-slate-700 hover:text-brand-300">
      {label}
    </a>
  );
}
  ...
  if (clubQuery.isError) {
    ...
    return (
      <div className="mx-auto max-w-3xl px-4 py-16 text-center">
        <h1 className="text-xl font-semibold text-white">{status === 404 ? "Club not found" : "Something went wrong"}</h1>
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 animate-fade-in">
      <div className="surface flex flex-col gap-4 p-6 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex gap-4">
          ...
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm text-slate-400">{club.description}</p>
      ...
      {club.facultyDetails && (
        <p className="mt-4 text-sm text-slate-500">
          <span className="font-medium text-slate-400">Faculty:</span> {club.facultyDetails}
        </p>
      )}
      ...
      <section className="mt-8">
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to={`/events?clubId=${club.id}`} className="text-sm text-slate-500 hover:text-slate-300">
              View all
            </Link>
          </div>
        </div>
      ...
      <section className="mt-8">
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to={`/projects?clubId=${club.id}`} className="text-sm text-slate-500 hover:text-slate-300">
              View all
            </Link>
          </div>
        </div>
      ...
      <section className="mt-8">
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to={`/blogs?clubId=${club.id}`} className="text-sm text-slate-500 hover:text-slate-300">
              View all
            </Link>
          </div>
        </div>
      ...
      <section className="mt-8">
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link to={`/gallery?clubId=${club.id}`} className="text-sm text-slate-500 hover:text-slate-300">
              View all
            </Link>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            {galleryQuery.data.items.map((item) => (
              <div key={item.id} className="surface overflow-hidden rounded-xl border border-slate-800 p-0 hover:border-slate-700 transition">
                ...
                />
                {item.caption && (
                  <p className="p-3 text-xs text-slate-300 truncate">{item.caption}</p>
                )}
              </div>
            ))}
          </div>
