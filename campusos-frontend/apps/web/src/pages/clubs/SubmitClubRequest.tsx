...
export function SubmitClubRequest() {
      ...
      <div className="mx-auto max-w-xl">
        <div className="surface p-6">
          <div className="flex items-center justify-between">
            <h1 className="text-lg font-semibold text-white">{request.clubName}</h1>
            <RequestStatusBadge status={request.status} />
          </div>
          <p className="mt-3 text-sm text-slate-400">{request.description}</p>
          <dl className="mt-4 space-y-3 text-sm">
            <div>
              <dt className="font-medium text-slate-400">Faculty details</dt>
              <dd className="text-slate-500">{request.facultyDetails}</dd>
            </div>
            <div>
              <dt className="font-medium text-slate-400">Reason</dt>
              <dd className="text-slate-500">{request.reason}</dd>
            </div>
            {request.status === "REJECTED" && request.rejectionReason && (
              <div>
                <dt className="font-medium text-slate-400">Rejection reason</dt>
  ...
  return (
    <form onSubmit={handleSubmit} className="surface mx-auto max-w-xl space-y-4 p-6">
      <div>
        <h1 className="text-lg font-semibold text-white">Request a new club</h1>
        <p className="mt-1 text-sm text-slate-500">A Super Admin will review this and assign a Faculty Coordinator if approved.</p>
      </div>
