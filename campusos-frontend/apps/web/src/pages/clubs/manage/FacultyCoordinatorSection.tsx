...
export function FacultyCoordinatorSection({ club }: FacultyCoordinatorSectionProps) {
  ...
  return (
    <div className="surface p-6">
      <h2 className="text-base font-semibold">Faculty Coordinator</h2>
      <p className="mt-1 text-sm text-muted">
        Current coordinator ID: <span className="font-mono text-xs text-muted">{club.facultyCoordinatorId ?? "Unassigned"}</span>
      </p>
        ...
        {searchTerm && (
          <div className="surface mt-2 divide-y divide-slate-800">
            ...
            {searchResults.data && candidates.length === 0 && (
              <p className="px-3 py-2 text-sm text-muted">No matching Faculty Coordinators.</p>
            )}
          </div>
        )}
      </div>
