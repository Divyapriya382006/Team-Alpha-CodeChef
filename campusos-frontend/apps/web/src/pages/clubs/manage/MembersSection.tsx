    ...
    { key: "role", header: "Role", render: (m) => <RoleBadge role={m.role} /> },
    {
      ...
      render: (m) => (
        <div className="flex justify-end gap-3">
          {m.role === "CLUB_HEAD" ? (
            <span className="text-xs text-muted">Club Head</span>
          ) : (
            <button type="button" onClick={() => handleDemote(m.userId)} className="text-xs font-medium text-muted hover:text-ink">
              Demote
            </button>
          )}
          {m.userId === user?.id ? (
